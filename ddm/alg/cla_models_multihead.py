...
###ALL OF THESE FUNCTIONS ARE NOT USED!!!!!!!!
...
############END OF FUNCTIONS THAT ARE NOT ACTUALLY USED!##############
...
class Cla_NN(object):
    ...
    def train(self, x_train, y_train, task_idx, no_epochs=1000, batch_size=100, display_epoch=5):
        ...
        for epoch in range(no_epochs):
            ...
            perm_inds = np.arange(x_train.shape[0], dtype=int)
...
#When uncommented, this calculates the KL divergence with a laplacian prior for the first task, and the usual gaussian gaussian KL divergence otherwise
    def _KL_term(self):
        kl = 0
        #THIS IS ONLY TRUE FOR THE VERY FIRST CALL OF THE _KL_term
        no_tasks = len(self.W_last_m)
        print('no_tasks at start of _KL_term call:' , no_tasks)
        if no_tasks == 1:
            for i in range(self.no_layers-1):
                din = self.size[i]
                dout = self.size[i+1]
                
                #cummulative kl for weights of entire layer
                m, v = self.W_m[i], self.W_v[i]
                m0, v0 = self.prior_W_m[i], self.prior_W_v[i]
                
                loc_diff = m - m0
                loc_diff_scale_ratio = loc_diff / tf.sqrt(tf.exp(v))
                
                t1 =  0.5*tf.reduce_sum(v - np.log(v0))
                t2 = tf.reduce_sum(tf.sqrt(2*tf.exp(v)/ np.pi) * tf.exp(-0.5*loc_diff_scale_ratio**2)/np.sqrt(v0))
                t3 = tf.reduce_sum(loc_diff * tf.erf(tf.sqrt((1/2*tf.exp(v))) * loc_diff) / np.sqrt(v0))
                t4 = dout * din * 0.5 * (1 + np.log(0.5 * np.pi))
        ...
        if no_tasks != 1:
            for i in range(self.no_layers-1): # THIS IS CONSTANT self.no_layers -1  is  2
                din = self.size[i]
                dout = self.size[i+1]
                m, v = self.W_m[i], self.W_v[i]
                m0, v0 = self.prior_W_m[i], self.prior_W_v[i]
            ...
            print('calculation of the kl for the parameters EXCL LAST has ended for this call of _KL_term. The kl for the parameters EXC LAST is', kl) 

            no_tasks = len(self.W_last_m)
            din = self.size[-2]
            dout = self.size[-1]
        
            for i in range(no_tasks):  #THIS CHANGES EACH TIME, STARTS AT 1 THEN 2 THEN 3 ... THEN 5.
                m, v = self.W_last_m[i], self.W_last_v[i]
                m0, v0 = self.prior_W_last_m[i], self.prior_W_last_v[i]
                const_term = -0.5 * dout * din
                log_std_diff = 0.5 * tf.reduce_sum(np.log(v0) - v)
...
#this calculates the KL divergence with a Gaussian prior
...
#         print('no_tasks at start of _KL_term call:' , no_tasks)
#         
#         print('kl_term has just been called')
#         kl = 0
#             
#         for i in range(self.no_layers-1): # THIS IS CONSTANT self.no_layers -1  is  2
#             din = self.size[i]
...
#             kl += const_term + log_std_diff + mu_diff_term
#             
#         #print('calculation of the kl for the parameter has ended for this call of _KL_term. The kl for the parameters is', kl) 
#         
#         return kl
