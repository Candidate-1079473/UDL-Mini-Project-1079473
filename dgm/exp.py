...
data_path = '/content/variational-continual-learning/ddm/data/mnist.pkl.gz' #this is actually only use for notMNIST 

def main(data_name, method, dimZ, dimH, n_channel, batch_size, K_mc, checkpoint, lbd):
    # set up dataset specific stuff
    from config import config    labels, n_iter, dimX, shape_high, ll = config(data_name, n_channel)
    if data_name == 'mnist':
    ...
    # File name
    fname = os.path.join(directory, data_name + '_%s.pkl' % string)
...
if __name__ == '__main__':
    ...
    if method == 'onlinevi' or method == 'noreg':
