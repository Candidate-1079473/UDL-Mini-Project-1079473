...
data_path = '/content/variational-continual-learning/ddm/data/mnist.pkl.gz'
...
def main(data_name, method, dimZ, dimH, n_channel, batch_size, K_mc, checkpoint, lbd):
    ...
    if method == 'onlinevi' and K_mc > 1:
        #print('yes method is onlinevi and K_mc > 1:')
        string = string + '_K%d' % K_mc
        
    #path_name = data_name + '_%s_no_share_enc/' % string
            
    path_name = data_name + '_' + string + '/'
    #print(path_name)
    ...
    for task in range(1):
        ...
        print('N_test / batch_size is ', N_test / batch_size)
